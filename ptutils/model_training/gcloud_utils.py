...
def lookup_tpu_ip_by_name(tpu_name, tpu_zone="us-central1-b"):
    gcloud_cmd = f"/usr/bin/gcloud compute tpus describe {tpu_name} --zone={tpu_zone}"
    ...
    ip = str(out).split("ipAddress: ")[1].split("\\n")[0]
    return ip


def configure_env_for_tpu(tpu_ip, tpu_vm=False):
    if tpu_vm:
        os.environ["XRT_TPU_CONFIG"] = f"localservice;0;localhost:51011"
    else:
...
def configure_tpu(tpu_name, tpu_zone):
